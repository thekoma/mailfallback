...
- **Mail index recipients (migration 029)**: `mail_index.messages` stores `to_addrs`, `cc_addrs`, `bcc_addrs` (Bcc only exists in the sender's own copy), all in the `tsv` trigger. `recipients_indexed_at` NULL marks a row indexed before Cc/Bcc were: `upsert_message_set` re-reads its headers once when the walk meets its file. Preview (`get_message`) reads recipients from the message bytes, so it is complete even for rows the walk can't re-read (snapshot-only)
...
- **ACL read-only**: `acl_driver = vfile`, `acl_globals_only = yes`. ACLs are settings blocks (dovecot 2.4.3+ removed the global acl file): a global `acl readonly { acl_id = owner; acl_rights = lrs }` makes every mailbox owner-read-only (incl. dynamic per-account namespaces), and `mailbox MFB-Staging` / `mailbox MFB-Staging/*` filters grant `lrwstie` on the staging mailbox that lives inside the root namespace (`{home}/root-inbox/MFB-Staging` — there is no separate per-user staging namespace). `acl_defaults_from_inbox = yes` makes defaults for mailboxes without an ACL entry come from INBOX (`lrs`) rather than the private-namespace default of full owner rights, so a client can't CREATE an undeletable top-level mailbox. Non-obvious: Dovecot's `mailbox` ACL filters match the namespace-INTERNAL mailbox name with the namespace prefix stripped — this is why the staging mailbox had to move inside the root namespace rather than living in its own, and also why it is named `MFB-Staging` and not `Staging`: the matcher is namespace-blind, so the plain name matched a *provider* folder a user happened to call Staging and handed out expunge rights over real backed-up mail (#237). No filter syntax separates the two; the name is the defence. The single source is `constants.STAGING_MAILBOX`, shared by the Maildir path, the ACL filter and the webmail deep link. Blocks delete, expunge, insert, flag changes everywhere except the staging mailbox. mbsync writes directly to filesystem — unaffected by ACLs.
