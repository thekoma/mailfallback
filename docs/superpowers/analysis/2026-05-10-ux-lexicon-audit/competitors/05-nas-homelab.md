...
**Mailcow** (Postfix + Dovecot + Rspamd + SOGo) uses *"backup"* exclusively — no separate "archive" product. The helper `backup_and_restore.sh` writes an rsync of `vmail/`, dumps MariaDB / Redis / crypt material, and recommends ZFS / Btrfs snapshots *on the destination*. Language is operational: *"Backup all, delete backups older than n days"* — rotation, not preservation. Built-in archive UX is absent; users get pushed to **Mailpiler-as-a-second-server** alongside Mailcow with SMTP journaling (community guides exist titled *"Self-Hosted Email Archiving with Mailpiler & Mailcow"*).

**NethServer (NS8)** ships **Piler** as a separate app: *"Piler is an open-source mail archiving solution. This Piler application for NS8 configures a Mail server instance to act as its archive."* Note the role separation — there is a Mail server *and* an archive, the archive has its own *"auditor users"* role, and the admin **cannot read** archived messages. This is the compliance framing: archive = legal/discovery-grade record with retention duration in days.

Takeaway: "archive" — as understood by a Mailcow / NethServer admin — implies SMTP journaling, auditor role, retention policy, immutability. MFB has none of these.
...
- **Thunderbird ImportExportTools NG** describes itself as *"various tools for importing and exporting messages."* UI verbs are **Export** / **Import**; **"backup"** only appears at the profile level (*"Auto profile backup with schedule on shutdown"*). Community articles then relabel the same operation as **"archive"**: *"Save an email archive in MBOX"*. So consumer "archive" = a folder of exported mbox/eml files kept somewhere safe.
- **Proton Mail Export Tool** is *"It helps you export decrypted emails from your Proton Mail account to your device."* — but the CLI prompts you to type **B** for **Backup** or **R** for **Restore**. Three words for one operation: marketing says **export**, CLI says **backup**, users say **download my mail**.
- **Tutanota / Tuta** has no first-class export. The r/tutanota pattern: *"select all, three-dot menu, Export → zip of .eml files."* Verbs in the wild: **export**, **archival export**, *"backup before final account closure"*, *"keep a copy"*.
- **imapsync** (closest CLI ancestor of MFB) self-describes as *"an IMAP transfer tool. The purpose of imapsync is to migrate IMAP accounts or to backup IMAP accounts."* The man page extends to *"syncing, copying, migrating and archiving … between two imap servers."* But the site explicitly notes *"Imapsync can't backup nor restore … to or from a local …"* — the moment the destination is *local*, "backup" becomes contested.
