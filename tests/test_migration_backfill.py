...
def test_migration_024_adds_total_folders_nullable(tmp_path, monkeypatch):
    """024 adds accounts.initial_sync_total_folders (nullable, no backfill —
    NULL = unknown until the next STATUS pass fills it). Existing rows stay
    NULL after the upgrade."""
    monkeypatch.delenv("MAILFALLBACK_DATABASE_URL", raising=False)
    db_url = f"sqlite:///{tmp_path}/mig24.db"
    cfg = _alembic_config(db_url)
